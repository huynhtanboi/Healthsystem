...
const MyShedule = () => {
  ...
  onst [timeSlots, setTimeSlots] = useState([]);
  ...
  const handleVaccineSelection = (vaccine) => {
    ...
    fetch('http://localhost:3600/record', {//placehold wait for record and fix later
      method: 'POST',
      headers: {
  ...
  rereturn (
    ...
    <div className="schedule-board">
      ...
      <div className="vaccine-board">
        ...
        <button
          className={`vaccine-btn ${selectedVaccine === 'Vaccine A' ? 'selected' : ''}`}
          onClick={() => handleVaccineSelection('Vaccine A')}
        >
          Vaccine A
        </button>
        <button
          className={`vaccine-btn ${selectedVaccine === 'Vaccine B' ? 'selected' : ''}`}
          onClick={() => handleVaccineSelection('Vaccine B')}
        >
          Vaccine B
        </button>
        <button
          className={`vaccine-btn ${selectedVaccine === 'Vaccine C' ? 'selected' : ''}`}
